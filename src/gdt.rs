...
pub const fn seg_desctype(x: u8) -> u8 {
    (x & 1) << SEG_DESCTYPE_BIT
}
#[inline]
/// Present
pub const fn seg_pres(x: u8) -> u8 {
    (x & 1) << SEG_PRES_BIT
}
#[inline]
/// Set privilege level (0 - 3)
pub const fn seg_priv(x: u8) -> u8 {
    (x & 0x03) << SEG_PRIV_SHIFT
} // Set privilege level (0 - 3)

// In the granularity flags
#[inline]
/// Long mode
pub const fn seg_long(x: u8) -> u8 {
    (x & 1) << SEG_LONG_BIT
}
#[inline]
/// Size (0 for 16-bit, 1 for 32)
pub const fn seg_size(x: u8) -> u8 {
    (x & 1) << SEG_SIZE_BIT
}
#[inline]
/// Granularity (0 for 1B - 1MB, 1 for 4KB - 4GB)
pub const fn seg_gran(x: u8) -> u8 {
    (x & 1) << SEG_GRAN_BIT
}
...
pub const GDT_ACCESS_CODE_PL0: u8 =
    (seg_desctype(1) | seg_pres(1) | seg_priv(0)) as u8 | SEG_CODE_EXRD;
pub const GDT_ACCESS_DATA_PL0: u8 =
    (seg_desctype(1) | seg_pres(1) | seg_priv(0)) as u8 | SEG_DATA_RDWR;
// Expand down flags aren't needed because we're not doing segmented memory, but flat paging instead
pub const GDT_ACCESS_STACK_PL0: u8 =
    (seg_desctype(1) | seg_pres(1) | seg_priv(0)) as u8 | SEG_DATA_RDWR;
pub const GDT_ACCESS_CODE_PL3: u8 =
    (seg_desctype(1) | seg_pres(1) | seg_priv(3)) as u8 | SEG_CODE_EXRD;
pub const GDT_ACCESS_DATA_PL3: u8 =
    (seg_desctype(1) | seg_pres(1) | seg_priv(3)) as u8 | SEG_DATA_RDWR;
pub const GDT_ACCESS_STACK_PL3: u8 =
    (seg_desctype(1) | seg_pres(1) | seg_priv(3)) as u8 | SEG_DATA_RDWR;
