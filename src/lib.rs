...
pub extern "C" fn kmain() -> ! {
    ...
    unsafe {
        ...
        gdt::write_gdt_entry(
            1,
            0xffff,
            gdt::GDT_ACCESS_CODE_PL0,
            gdt::GDT_SEG_GRANULAR_FLAGS,
        );
        gdt::write_gdt_entry(
            2,
            0xffff,
            gdt::GDT_ACCESS_DATA_PL0,
            gdt::GDT_SEG_GRANULAR_FLAGS,
        );
        gdt::write_gdt_entry(
            3,
            0xffff,
            gdt::GDT_ACCESS_STACK_PL0,
            gdt::GDT_SEG_GRANULAR_FLAGS,
        );
        gdt::write_gdt_entry(
            4,
            0xffff,
            gdt::GDT_ACCESS_CODE_PL3,
            gdt::GDT_SEG_GRANULAR_FLAGS,
        );
        gdt::write_gdt_entry(
            5,
            0xffff,
            gdt::GDT_ACCESS_DATA_PL3,
            gdt::GDT_SEG_GRANULAR_FLAGS,
        );
        gdt::write_gdt_entry(
            6,
            0xffff,
            gdt::GDT_ACCESS_STACK_PL3,
            gdt::GDT_SEG_GRANULAR_FLAGS,
        );
        gdt::reload_gdt(7);
    }
    ...
    pr_debug!(
        "Address of test string: 0x{:08x}",
        &raw const test_stack as u32
    );
